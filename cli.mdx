...
Check that an artist has the required assets (face-guide, songs, context files) before creating content.

```bash
recoup content validate --artist <artistSlug>
recoup content validate --artist <artistSlug> --json
```

| Flag | Required | Description |
|------|----------|-------------|
| `--artist <slug>` | Yes | Artist slug |

### Estimate cost

Preview the estimated cost and duration for a content run without starting it.

```bash
recoup content estimate --artist <artistSlug>
recoup content estimate --artist <artistSlug> --template <name> --json
```
...
Trigger the full content-creation pipeline. Returns a run ID you can check with `recoup tasks status`.

```bash
recoup content create --artist <artistSlug>
recoup content create --artist <artistSlug> --template <name> --lipsync --upscale
recoup content create --artist <artistSlug> --json
```
...
This command wraps [`POST /api/content`](/api-reference/content/create).
